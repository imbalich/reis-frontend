...
import {
  createPartFittingAaronApi,
  createProductFittingAaronApi,
  getProductRunTimeParamsApi,
  queryPartCalculateApi,
  queryPartFittingAaronApi,
  queryProductFittingAaronApi,
} from '#/api';
import DistributionFactory from '#/custom/weibull/factories/distrbution-fact';

import { calculateSchema, DEFAULT_PRODUCT_CONFIG_CODE, schema } from './data';
  ...
  handleValuesChange: async (_, fieldsChanged) => {
    if (fieldsChanged.includes('model')) {
      await formApi.setValues({
        product_config_code: DEFAULT_PRODUCT_CONFIG_CODE,
        part: undefined,
      });
      return;
    }
...
const normalizeProductConfigCode = (value?: string) =>
  value === DEFAULT_PRODUCT_CONFIG_CODE ? '' : (value ?? '');
...
const handleSubmit = async () => {
  ...
  createData.product_config_code = normalizeProductConfigCode(
    createData.product_config_code,
  );
  queryData.product_config_code = normalizeProductConfigCode(
  ...
  try {
    ...
    const res = queryData.part
      ? await queryPartFittingAaronApi(queryData)
      : await queryProductFittingAaronApi(queryData);
    ...
    await (queryData.part
      ? createPartFittingAaronApi(createData)
      : createProductFittingAaronApi(createData));
...
const handleCalculateQuery = async () => {
  ...
  const params = {
    ...
    product_config_code: normalizeProductConfigCode(mainValues.product_config_code),
