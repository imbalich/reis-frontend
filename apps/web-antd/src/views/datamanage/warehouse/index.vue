...
import {
    getWarehouseListApi,
    importWarehouseDataApi,
    type GetWarehouseDetails,
    type WarehouseFormImportParams
} from '#/api/warehouse';
...
const gridOptions: VxeTableGridOptions<GetWarehouseDetails> = {
    rowConfig: {
        keyField: 'id',
    },
    checkboxConfig: {
    ...
    proxyConfig: {
        ajax: {
            query: async ({ page }, formValues) => {
                ...
                return await getWarehouseListApi(params);
            },
        },
    },
};
...
const [ImportModal, importModalApi] = useVbenModal({
    ...
    async onConfirm() {
        ...
        importModalApi.lock();
        try {
            const formValues = await importFormApi.getValues<WarehouseFormImportParams>();
            ...
            const result = await importWarehouseDataApi({
...
async function downloadTemplate() {
    try {
        // 直接下载预置的XLSX模板文件
        const templateUrl = '/库房二级配属对应关系配置.xlsx';
        const link = document.createElement('a');
        link.href = templateUrl;
        link.download = '库房二级配属对应关系配置.xlsx';
